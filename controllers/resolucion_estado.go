...
// @Param	body		body 	models.ResolucionEstado	true		"body for ResolucionEstado content"
// @Success 201 {int} models.ResolucionEstado
// @Failure 400 the request contains incorrect syntax
// @router / [post]
func (c *ResolucionEstadoController) Post() {
	var v models.ResolucionEstado
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		v.FechaCreacion = time_bogota.TiempoBogotaFormato()
		v.FechaModificacion = time_bogota.TiempoBogotaFormato()
		if _, err := models.AddResolucionEstado(&v); err == nil {
...
// @Success 200 {object} models.ResolucionEstado
// @Failure 404 not found resource
// @router /:id [get]
func (c *ResolucionEstadoController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetResolucionEstadoById(id)
	if err != nil {
		logs.Error(err)
...
func (c *ResolucionEstadoController) GetAll() {
	...
	l, err := models.GetAllResolucionEstado(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *ResolucionEstadoController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.ResolucionEstado{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		v.FechaCreacion = time_bogota.TiempoCorreccionFormato(v.FechaCreacion)
		v.FechaModificacion = time_bogota.TiempoBogotaFormato()
		if err := models.UpdateResolucionEstadoById(&v); err == nil {
...
func (c *ResolucionEstadoController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteResolucionEstado(id); err == nil {
